...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    if let path = Bundle.main.path(forResource: "App-Config", ofType: "plist") {
      if let root = NSDictionary(contentsOfFile: path){
        if let refreshRate = root.value(forKey: "refresh_rate") as? String {
          UIApplication.shared.setMinimumBackgroundFetchInterval(Double(refreshRate)!)
        }
      }
    }
  ...
  func application(_ application: UIApplication, performFetchWithCompletionHandler completionHandler: @escaping (UIBackgroundFetchResult) -> Void) {
    if let path = Bundle.main.path(forResource: "Api-Params", ofType: "plist") {
      if let root = NSDictionary(contentsOfFile: path){
        ClientApiCallService.shared.getRateApi(selectedCurrencyCode: root.value(forKey: "source") as? String ?? "USD",completion: {
          result in
          print("\(String(describing: result.value?.sourceCurrency))")
        })
      }
    }
